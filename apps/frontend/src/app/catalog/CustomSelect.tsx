...
export default function CatalogSidebar() {
  ...
  const collections = gift.collections as GiftItem[]
  const collection = collections.find(c => c.id === searchParams.get('collection'))
  const models = (collection?.models || []) as GiftItem[]
  const symbols = gift.symbols as GiftItem[]
  const backdrops = gift.backdrops as GiftItem[]
  ...
  const getImage = ({ id, collection }: Pick<CustomSelect, 'id' | 'collection'>) => {
    let result = id
...
<<<<<<< HEAD
  ...
  const filterItems = <T extends GiftItem>(items: T[], query: string): T[] => {
