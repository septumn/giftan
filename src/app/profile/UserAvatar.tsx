...
import { deleteAvatarAction } from "../../actions/avatar-actions"
...
const UserAvatar = () => {
  ...
  const currentImage = optimisticAvatar || (session?.user?.image
    ? `${process.env.NEXT_PUBLIC_AVATARS_URL}/${session.user.image}`
    : null)
  ...
  return (
    <div className={styles.avatarWrap}>
      ...
      />
      {session?.user?.emailVerified ? <div className={styles.avatarBadge}>✓</div> : ''}
    </div>
  )
}
